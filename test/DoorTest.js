...
describe("Door contract tests", function () {
    ...
    beforeEach(async function () {
        [deployer, witness1, witness2, Alice, Bob] = await ethers.getSigners()
        ...
        //console.log(`deployer ${deployer.address} Alice ${Alice.address} Bob ${Bob.address}`)
    })
    ...
    it("claimId event", async function () {
        const BobAddress = Bob.address.toString().toLowerCase()
    ...
    it("commit events", async function () {
        ...
        await expect(door2.connect(Alice).XChainCommit(1, sigRewardMin, BobAddress, { value: transferMin }))
            .to.emit(door2, "XChainCommitEvent")
            .withArgs(1, 1, transferMin, sigRewardMin, AliceAddress, BobAddress);

        await expect(door2.connect(Alice).XChainCreateAccountCommit(sigRewardMin, BobAddress, { value: accountCreateMin }))
            .to.emit(door2, "XChainCommitEvent")
            .withArgs(0, 1, accountCreateMin, sigRewardMin, AliceAddress, BobAddress);
    })
